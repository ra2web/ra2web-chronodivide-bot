...
async function main() {
    ...
    mp02t2.map,mp06t2.map,mp11t2.map,mp08t2.map,mp21s2.map,mp14t2.map,mp29u2.map,mp31s2.map,mp18s3.map,mp09t3.map,mp01t4.map,mp03t4.map,mp05t4.map,mp10s4.map,mp12s4.map,mp13s4.map（蒙大拿非军事区）,mp19t4.map,
    mp15s4.map,mp16s4.map,mp23t4.map,mp33u4.map,mp34u4.map,mp17t6.map,mp20t6.map（狂野战区）,mp25t6.map（防卫区争夺）,mp26s6.map（白令海峡）,mp30s6.map（西伯利亚荒原）,mp22s8.map（北极圈）,mp27t8.map（长征）,mp32s8.map,mp06mw.map,mp08mw.map,mp14mw.map,mp29mw.map,
    mp05mw.map,mp13mw.map,mp15mw.map,mp16mw.map,mp23mw.map(正面冲击-巨富),mp17mw.map(马里布悬崖-巨富),mp25mw.map,mp30mw.map,mp22mw.map,mp27mw.map,mp32mw.map,mp09du.map,mp01du.map,mp05du.map,mp13du.map,mp15du.map,mp18du.map,
    ...
    newhghts.map,oceansid.map（海滩）,pacific.map,potomac.map,powdrkeg.map,rockets.map,roulette.map,round.map,seaofiso.map,shrapnel.map,tanyas.map,tower.map,tsunami.map,valley.map,xmas.map,yuriplot.map,
    ...
    */
    ...
    const onlineSettings: CreateOnlineOpts = {
        ...
        agents: [
            new SupalosaBot(process.env.ONLINE_BOT_NAME ?? botName1, Countries.RUSSIA).setDebugMode(true),
    ...
    const offlineSettings1v1: CreateOfflineOpts = {
        ...
        online: false,
        agents: [
            new SupalosaBot(botName1, Countries.FRANCE, [], false),
            new SupalosaBot(botName4, Countries.RUSSIA, [], true).setDebugMode(true),
        ],
    };
    ...
    const offlineSettings2v2: CreateOfflineOpts = {
        ...
        online: false,
        agents: [
            new SupalosaBot(botName1, Countries.IRAQ, [botName2], false),
            new SupalosaBot(botName2, Countries.RUSSIA, [botName1], true).setDebugMode(true),
            new SupalosaBot(botName3, Countries.USA, [botName4], false),
